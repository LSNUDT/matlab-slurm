...
%additionalSubmitArgs = sprintf('--cpus-per-task=%d', cluster.NumThreads);
additionalSubmitArgs = ''; %sprintf('--nodes=2 --tasks-per-node=3 --mem=4GB')
...
commandToRun = sprintf('bash %s', localScriptName);

fprintf('Command to run: %s\n', commandToRun);
